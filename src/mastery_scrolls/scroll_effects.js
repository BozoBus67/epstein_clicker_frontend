...
export function apply_building_buffs(entries, pgd) {
  return entries.map(entry => {
    ...
    // Diddy: ×2500 on Diddy-themed buildings (2, 3, 4)
    if ((pgd.diddy ?? 0) > 0 && DIDDY_BUILDINGS.includes(entry.key)) {
      cps *= 2500;
    }
...
export function apply_global_modifiers(total_cps, pgd) {
  ...
  // Shadow Clone Jutsu (image: blurry_epstein): ×1000 over total cps
  if ((pgd.blurry_epstein ?? 0) > 0) {
    cps *= 1000;
  }
